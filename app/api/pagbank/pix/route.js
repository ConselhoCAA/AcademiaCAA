...
export async function POST(request){
 try{
  ...
  const {data:existing}=await admin.from('payments').select('id,status,pix_code,qr_code_url,expires_at,provider_payment_id').eq('enrollment_id',e.id).eq('provider','pagbank').in('status',['pending','processing']).order('created_at',{ascending:false}).limit(1).maybeSingle();
  ...
  const body={reference_id:e.id,items:[{reference_id:e.courses.id,name:e.courses.title,quantity:1,unit_amount:e.courses.price_cents}],charges:[{reference_id:e.id,description:e.courses.title,amount:{value:e.courses.price_cents,currency:e.courses.currency||'BRL'},payment_method:{type:'PIX',pix:{expiration_date:expires}}}],notification_urls:[`${appUrl}/api/pagbank/webhook`]};
  ...
  if(!response.ok) return NextResponse.json({error:'O PagBank não autorizou a criação do PIX.',details:pg?.error_messages?.map(x=>x.description).filter(Boolean)},{status:502});
  ...
  if(saveError) return NextResponse.json({error:'PIX criado, mas não foi possível registrar a cobrança.'},{status:500});
