...
export default function Painel(){
 ...
 useEffect(()=>{(async()=>{const supabase=getSupabaseBrowserClient();const {data:u}=await supabase.auth.getUser();if(!u.user){window.location.href='/aluno';return;}setUser(u.user);
  let {data,error}=await supabase.from('enrollments').select('id,status,enrolled_at,courses(id,slug,title,price_cents,starts_at,modality)').order('enrolled_at',{ascending:false});
  if(!error&&(!data||data.length===0)){
   const pending=localStorage.getItem('academia_caa_pending_course');
   const courseSlug=pending||COURSE;
   const {error:enrollError}=await supabase.rpc('create_enrollment',{course_slug:courseSlug});
   if(!enrollError){localStorage.removeItem('academia_caa_pending_course');const refreshed=await supabase.from('enrollments').select('id,status,enrolled_at,courses(id,slug,title,price_cents,starts_at,modality)').order('enrolled_at',{ascending:false});data=refreshed.data;error=refreshed.error;}
  }
  if(error)setError('Não foi possível carregar sua matrícula.');else setItems(data||[]);setLoading(false);})();},[]);
