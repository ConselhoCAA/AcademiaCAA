...
export default function SignupForm(){
 ...
 async function submit(e){
  ...
  setBusy(true);setMsg('');
  const f=new FormData(form);const email=String(f.get('email')||'').trim();const password=String(f.get('password')||'');const full_name=String(f.get('full_name')||'').trim();const certificate_name=String(f.get('certificate_name')||'').trim();
  try{
   ...
   if(data.session){
    ...
    window.location.href='/aluno/painel';return;
   }
   setMsg('Cadastro realizado. Confirme seu e-mail e depois entre na Área do Aluno para concluir automaticamente sua matrícula.');
   form?.reset?.();
  }catch(err){setMsg(signupMessage(err));}finally{setBusy(false);}
 }
